...
const Form = () => {
    ...
    const formSchema = yup.object().shape({
        ...
        password: yup.string().required("Password required upon signup").max(8, "Password is too short, needs at least 8 characters"),
...
return (
    <form onSubmit={formSubmit}>
        ...
        <label htmlFor="email">
            Email:
            <input data-cy="email" id="email" type="text" name="email" value={formState.email} onChange={inputChange}/>
            {errors.email.length > 0 ? (<p className="error">{errors.email}</p>) : null}
        </label>
        
        <label htmlFor="password">
            Password:
            <input data-cy="password" id="password" type="text" name="password" value={formState.password} onChange={inputChange}/>
